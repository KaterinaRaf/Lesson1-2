...
int GetDeterminate2x2(int[,] minor) //определитель матрицы
{
...
{
    return 3; //нужна верная формула. (используем формулу из 2*2 выше)
}

//метод, возращающий массив 2х2 из массива 3х3, в котором удалены строки и столбцы по индексам
//вычеркиваем нечетные столбцы и нечетные строки
int[,] DeleteLines(int[,] arr, int posRow, int posColumn)
{
  ...
  for (int i = 0; i < arr.GetLength(0); i++)
  {
    if (i != posRow)
    {
        for (int j = 0; j < arr.GetLength(1); j++)  //цикл по строке
        {
            if (j != posColumn)
            {
        ...
        l = 0; 
        k++;  //индекс эл-та в новом массиве
    }          
  }
    return resultArr;
}

int[,] GetMassivOfMinors(int[,] arr) //определяет минор матрицы
{
...
int[,] GetTranspositionMassiv(int[,] arr) // транспонирование минорной матрицы
{
    int[,] resultArr = new int[arr.GetLength(0), arr.GetLength(1)];
    for(int i = 0; i < arr.GetLength(0); i++)
    {
...
int[,] GetMultiMassivAndChislo(int[,] arr, int number) // умножение матриц на число
{
    int[,] resultArr = new int[arr.GetLength(0), arr.GetLength(1)];
    for(int i = 0; i < arr.GetLength(0); i++)
    {
...
int[,] GetMultiArrays(int[,] array1, int[,] array2) //умножение массива на массив
{
    ...
    for (int i = 0; i < array1.GetLength(0); i++)
    {
        for (int j = 0; j < array2.GetLength(1); j++)
        {
            ...
            {
                mult[i, j] += array1[i, k] * array2[k, j]; // += сложение!!!
            }
        }
    }
...
int[,] GetReverseArray(int[,] arr) // поиск обратной матрицы
{
    int[,] resultArr = new int[arr.GetLength(0), arr.GetLength(1)];
...
int[,] GetDivision(int[,] arrNom, int[,] arrDenom) // деление матриц
{
